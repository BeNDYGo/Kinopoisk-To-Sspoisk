<div align="center"> <h1>Kinopoisk-To-Sspoisk</h1></div>

![Downloads](https://img.shields.io/github/downloads/BeNDYGo/Kinopoisk-To-Sspoisk/total.svg)
![LastRelizDownloads](https://img.shields.io/github/downloads/BeNDYGo/Kinopoisk-To-Sspoisk/latest/total.svg)
![GitHub Repo stars](https://img.shields.io/github/stars/BeNDYGo/Kinopoisk-To-Sspoisk)

- ![TG-img](https://cdn-icons-png.freepik.com/16/15047/15047595.png) [KinipoiskToSspoisk](https://t.me/KinipoiskToSspoisk)
- ![Gmail-img](https://cdn-icons-png.freepik.com/16/5968/5968534.png?ga=GA1.1.1230537149.1769259151) bendygo6@gmail.com

Расширение для бесплатного просмотра фильмов на **kinopoisk** с возможностью откладывать фильмы в избранное. 

⭐*буду благодарен за звездочки*⭐

# Установка: 
<details>
<summary>Нажмите, чтобы раскрыть</summary>

#### Чтобы скачать расширение нужно нажать на последнюю версию в окне Releases в панели справа от текста, что вы сейчас читаете (или пройти по [ссылке](https://github.com/BeNDYGo/Kinopoisk-To-Sspoisk/releases)) и скачать последнюю версию расширения

![Screenshot_6](https://github.com/BeNDYGo/Kinopoisk-To-Sspoisk/blob/main/preview/Screenshot_6.png)

#### Чтобы установить расширение в браузер нужно следовать стандартой инструкции от chrome*:

1. Откройте настройки: Запустите Chrome, нажмите на значок меню (три точки в правом верхнем углу) ➔ выберите «Расширения» ➔ «Управление расширениями».
2. Включите режим разработчика: Переведите переключатель в Режим разработчика в правом верхнем углу страницы.
3. Загрузите папку: В левом верхнем углу нажмите кнопку «Загрузить распакованное расширение».
4. Укажите путь: В открывшемся окне выберите нужную папку с расширением и нажмите кнопку «Выбор папки» (или «ОК»).

Расширение появится в общем списке. После этого режим разработчика можно отключить, однако саму папку с компьютера удалять нельзя, иначе расширение перестанет работать.

---
*\*работает и для других браузеров*

</details>

# Примеры:

на страницу добавляется кнопка **Смотреть бесплатно**, **Смотреть позже** и **Меню**

- **Смотреть бесплатно** - открывает фильм для просмотра
- **Смотреть позже** - добавляет фильм в отложенное
- **Меню** - открывает меню

![Screenshot_1](https://github.com/BeNDYGo/Kinopoisk-To-Sspoisk/blob/main/preview/Screenshot_1.png)
![Screenshot_4](https://github.com/BeNDYGo/Kinopoisk-To-Sspoisk/blob/main/preview/Screenshot_4.png)
![Screenshot_2](https://github.com/BeNDYGo/Kinopoisk-To-Sspoisk/blob/main/preview/Screenshot_2.png)
![Screenshot_3](https://github.com/BeNDYGo/Kinopoisk-To-Sspoisk/blob/main/preview/Screenshot_3.png)


# Обновления:
>В случае выхода обновления вы будете видеть уведомление.

![Screenshot_5](https://github.com/BeNDYGo/Kinopoisk-To-Sspoisk/blob/main/preview/Screenshot_5.png)
# Конфиденциальность:

> Никакие данные не передаются!


# Работа:

> ### SSpoisk
> После нажатия на кнопку **Смотреть бесплатно** открывается сайт **flcksbr** с фильмом.
> 
> Добавляется кнопка **Назад**. Она открывает фильм **обратно**
> 
> *Для более приятного просмотра кнопка **прозрачная**, если не наводить на нее мышку*


# Связь со мной!
По **любым** вопросам/предложениям/вообще всему можете написать мне на [почту](bendygo6@gmail.com) или в [личное сообщение канала телеграмм](https://t.me/KinipoiskToSspoisk)(*там я отвечу быстрее*)

- https://t.me/KinipoiskToSspoisk
- bendygo6@gmail.com
---
